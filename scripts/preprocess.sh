...
do
  ...
  do
    ...
    cat work/tree_per_line/"${LANG}"/"${FOLD}"/rnng/test.json | python src/get_rnng_sentence.py > work/tree_per_line/"${LANG}"/"${FOLD}"/rnng/test.tokens
    python src/rnng-pytorch/scripts/dump_tokens.py work/tree_per_line/"${LANG}"/"${FOLD}"/tst.preterm > work/tree_per_line/"${LANG}"/"${FOLD}"/rnng/test.tokens
